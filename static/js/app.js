// Cliente JavaScript para SA-MP Radio Stream - Enhanced Version
...
let pauseBtn, resumeBtn, stopBtn, prevBtn, nextBtn;
let shuffleBtn, repeatBtn, themeToggle;
...
// ========================================
// INICIALIZACIÓN DE CARACTERÍSTICAS MEJORADAS
// ========================================
function initializeEnhancedFeatures() {
    // Initialize enhanced player
    enhancedPlayer = new EnhancedPlayer();
    
    // Initialize audio visualizer
    if (document.getElementById('audioVisualizer')) {
        visualizer = new AudioVisualizer('audioVisualizer');
    }
...
function toggleTheme() {
    if (enhancedPlayer) {
        enhancedPlayer.toggleTheme();
    }
}

function toggleShuffle() {
    if (enhancedPlayer) {
        const shuffleState = enhancedPlayer.toggleShuffle();
        showStatus(`<i class="fas fa-shuffle"></i> Modo aleatorio ${shuffleState ? 'activado' : 'desactivado'}`);
        
        // Update playlist when shuffle is toggled
        if (allSongs.length > 0) {
            enhancedPlayer.setPlaylist(allSongs);
        }
    }
}
...
function toggleRepeat() {
    if (enhancedPlayer) {
        const repeatMode = enhancedPlayer.toggleRepeat();
        let message = '';
        switch(repeatMode) {
            case 'off':
                ...
                break;
            case 'playlist':
                message = 'Repetir playlist';
                break;
            case 'song':
                message = 'Repetir canción';
                break;
        }
        showStatus(`<i class="fas fa-repeat"></i> ${message}`);
    }
}

async function playNextSong() {
    if (!enhancedPlayer || allSongs.length === 0) return;
    
    const nextTrack = enhancedPlayer.getNextTrack();
    if (nextTrack) {
        await playSong(nextTrack);
    } else {
        showStatus('<i class="fas fa-info-circle"></i> No hay siguiente canción');
    }
}
...
async function playPreviousSong() {
    ...
    const prevTrack = enhancedPlayer.getPreviousTrack();
    if (prevTrack) {
        await playSong(prevTrack);
    } else {
        showStatus('<i class="fas fa-info-circle"></i> No hay canción anterior');
    }
}
...
function initializeAudioVisualizer() {
    // The visualizer is now handled by the AudioVisualizer class
    // initialized in initializeEnhancedFeatures()
}

function resizeCanvas() {
    // Handled by AudioVisualizer class
}
...
function startVisualizer() {
    ...
    if (albumArtwork) {
        albumArtwork.classList.add('playing');
    }
}
...
function stopVisualizer() {
    ...
    // Stop album artwork rotation
    if (albumArtwork) {
        albumArtwork.classList.remove('playing');
    }
}
...
function initializeVolumeControl() {
    // Load saved volume from enhanced player
    if (enhancedPlayer) {
        const volume = enhancedPlayer.volume;
    ...
    volumeSlider.addEventListener('input', (e) => {
        ...
        if (enhancedPlayer) {
            enhancedPlayer.setVolume(volume);
        }
        updateVolumeDisplay(volume);
...
function updateVolumeDisplay(volume) {
    ...
    // Update icon using enhanced player if available
    if (enhancedPlayer) {
        enhancedPlayer.setVolume(volume);
    } else {
        // Fallback icon update
...
function toggleMute() {
    if (enhancedPlayer) {
        const newVolume = enhancedPlayer.toggleMute();
...
async function loadSongs() {
    try {
        ...
        if (response.ok) {
            ...
            // Update enhanced player playlist
            if (enhancedPlayer && allSongs.length > 0) {
                enhancedPlayer.setPlaylist(allSongs);
            }
...
async function playSong(nombre) {
    try {
        ...
        if (response.ok) {
            ...
            if (enhancedPlayer) {
                enhancedPlayer.setCurrentTrack(nombre);
            }
...
function updatePlayerState() {
    if (currentState.playing) {
        // Update song info in the new UI
        const songTitle = currentSongDiv.querySelector('.song-title');
        if (songTitle) {
            songTitle.innerHTML = `<i class="fas fa-music"></i> ${currentState.current_song}`;
        }
        
        const songArtist = currentSongDiv.querySelector('.song-artist');
        if (songArtist) {
            songArtist.textContent = currentState.paused ? '⏸ Pausado' : '▶ Reproduciendo';
        }
        ...
        // Enable next/prev buttons if there are songs
        ...
        // Update song info for no song playing
        const songTitle = currentSongDiv.querySelector('.song-title');
        if (songTitle) {
            songTitle.textContent = 'No hay canción seleccionada';
        }
        ...
        // Disable next/prev when not playing
